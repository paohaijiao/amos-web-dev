<template>
  <div style="min-height: 400px" :visible="dialogVisible">
    <div class="modal-header">
      <h4 class="modal-title">Hadoop file Output</h4>
    </div>
    ...
    <div class="form-group">
      <label for="name" >文件名<span style="color:red;font-size:12px;"> (样例: hdfs://root:13579admin@hadoop:9000/haha)</span></label>
      <input type="text" class="form-control"    v-model="form.file_name" placeholder="请输入文件名">
    </div>
    <div class="form-group">
      <label for="name" >分隔符 <span style="color:red;font-size:12px;"> (样例: ;)</span></label>
      <input type="text" class="form-control"    v-model="form.separator" placeholder="请输入分隔符">
    </div>

    <div class="form-group">
      <label for="name" >封闭符 <span style="color:red;font-size:12px;"> (样例: \)</span></label>
      <input type="text" class="form-control"    v-model="form.enclosure" placeholder="请输入封闭符">
    </div>
    <div class="form-group">
      <label for="name" >扩展名 </label>
      <input type="text" class="form-control"    v-model="form.file_extention" placeholder="请输入扩展名">
    </div>
    <div class="form-group">
      <label for="name" >头部 </label>
    ...
    <div class="form-group">
      <label for="name" >尾部 </label>
      <select  v-model="form.footer" class="form-control select2 select2-hidden-accessible">
        <option v-for="item in optiona"  :key="item.value" :label="item.name" :value="item.value"></option>
      </select>
    </div>
    <div class="form-group">
      <label for="name" >创建父目录 </label>
      <select  v-model="form.create_parent_folder" class="form-control select2 select2-hidden-accessible">
        <option v-for="item in optiona"  :key="item.value" :label="item.name" :value="item.value"></option>
      </select>
    </div>
