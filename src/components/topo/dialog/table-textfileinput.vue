<template>
  <div style="min-height: 400px" :visible="dialogVisible">
    <div class="modal-header">
      <h4 class="modal-title">TextFileInput</h4>
    </div>
    ...
    <div class="form-group">
      <label >文件类型</label>
      <select  v-model="form.file_type" class="form-control select2 select2-hidden-accessible">
        <option v-for="item in optionc"  :key="item.value" :label="item.name" :value="item.value"></option>
      </select>
    </div>
    <div class="form-group">
      <label for="name" >分隔符</label>
      <input type="text" class="form-control" id="name"   v-model="form.separator" placeholder="请输入分隔符">
    </div>
    <div class="form-group">
      <label for="name" >封闭符</label>
      <input type="text" class="form-control" id="name"   v-model="form.enclosure" placeholder="请输入封闭符">
    </div>
    <div class="form-group">
  ...
  export default {
    ...
    data() {
      return {
        ...
        optionc: [
          {name: 'txt', value: 'txt' },{name: 'CSV', value: 'CSV' }
        ],
        typeoptions: [],
        tableData: [],
        clusters:[],
